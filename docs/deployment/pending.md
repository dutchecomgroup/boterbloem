...
De ronde van **13-09** (teksten, taarten, slugs, logo, homepage) staat live, zie [history.md](history.md). Daarvóór is alles op **31-08 gedeployd**. De acht migraties, de content van de klant, de
...
| 🔴 **Agenda-token vernieuwen** | de beheerder: *Instellingen → Nieuwe link*. Het oude token is tot 13-09 aan elke bezoeker meegestuurd |
| 🟡 **Vanaf-vinkje bij Cupcakes en Mini dessert** | de klant: die staan sinds de migratie op *vanaf € 3,00* en *vanaf € 1,95* |
| 🟡 **Foto's bij de werkwijze-stappen** | de klant, op `/admin/teksten`. Of alleen `zetWerkwijze()` uit de seed draaien -- níet de hele seed, die overschrijft de hero-tekst |
| 🟡 **Beheerschermen doorklikken** | niets -- `/admin/teksten` en `/admin/producten` zijn niet met de hand getest, want ze zitten achter een login |
