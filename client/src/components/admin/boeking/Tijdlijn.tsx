...
export function Tijdlijn({ gebeurtenissen }: { gebeurtenissen: Gebeurtenis[] }) {
  ...
  if (gebeurtenissen.length === 0) {
    return (
      <p className="rounded-md bg-charcoal/[0.03] px-3 py-4 text-sm text-charcoal/70">
        ...
        {zichtbaar.map((g, i) => (
          <li key={g.id} className="relative flex gap-3 pb-3 last:pb-0">
            ...
            {i < zichtbaar.length - 1 && (
              <span className="absolute left-[3px] top-2 h-full w-px bg-charcoal/12" aria-hidden />
            )}
            <span
